...
int AES_CTR_test(){
    ...
    uint8_t iv1_128[16] = {0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA, 0xFB, 0xFC, 0xFD, 0xFE, 0xFF};
    ...
    crypt_AES_encrypt(key1_128, sizeof(key1_128), AES_128, iv1_128, sizeof(iv1_128), 
...
///////// test 2 //////////
    ...
    uint8_t iv2_128[] = {0x0A, 0x12, 0x3D, 0x56, 0x01, 0x11, 0xAE, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01};
    ...
    crypt_AES_encrypt(key2_128, sizeof(key2_128), AES_128, iv2_128, sizeof(iv2_128),  
    ...
    ///////// test 3 //////////
    uint8_t iv3_128[] = {0x0A, 0x12, 0x3D, 0x56, 0x01, 0x11, 0xAE, 0xFF, 0x91, 0xA7, 0x27, 0x61, 0xC7, 0x90, 0x0C, 0xEE};
    ...
    crypt_AES_encrypt(key3_128, sizeof(key3_128), AES_128, iv3_128, sizeof(iv3_128), 
    ...
    uint8_t iv4_192[] = {0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF, 0xCB, 0x6E, 0x5A, 0x2E, 0x27, 0x79, 0x28, 0x8D};
    ...
    crypt_AES_encrypt(key_192, sizeof(key_192), AES_192, iv4_192, sizeof(iv4_192),  
    ...
    uint8_t iv5_256[] = {0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0x01, 0xF2, 0xD9, 0xBF, 0xF4, 0x5A, 0x55, 0xCA, 0xCF};
    ...
    crypt_AES_encrypt(key_256, sizeof(key_256), AES_256, iv5_256, sizeof(iv5_256),  
...
int hmac_test(){
    ...
    crypt_hmac(key, sizeof(key) - 1, SHA256, text, sizeof(text) - 1, mac, sizeof(mac));
    failed |= assert_eq_texts(mac, expected_mac, 32, "HMAC-SHA256 test 1");

    crypt_hmac(key, sizeof(key) - 1, SHA512, text, sizeof(text) - 1, mac_512, sizeof(mac_512));
    failed |= assert_eq_texts(mac_512, expected_mac512, 64, "HMAC-SHA512 test 1");
...
int ghash_test(){
    ...
    const uint8_t expected1[] = {
        0xf7, 0xf4, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0e, 0xdd
    };
    GHASH(H1, C1, sizeof(C1), output, sizeof(output));
    failed |= assert_eq_texts(output, expected1, sizeof(expected1), "GHASH test 1");
    ...
    const uint8_t expected2[] = {
        0x91, 0xa4, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xcd
    };
    GHASH(H2, C2, sizeof(C2), output, sizeof(output));
    failed |= assert_eq_texts(output, expected2, sizeof(expected2), "GHASH test 2");
    ...
    const uint8_t expected3[] = {
        0xa6, 0x6e, 0x5c, 0x0a, 0x72, 0xa5, 0x70, 0xd9,
        0x69, 0x20, 0x17, 0xee, 0x37, 0x5c, 0x24, 0xba
    };
    GHASH(H3, C3, sizeof(C3), output, sizeof(output));
    failed |= assert_eq_texts(output, expected3, sizeof(expected3), "GHASH test 3");
    ...
    GHASH(H3, C4, sizeof(C4), output, sizeof(output));
    failed |= assert_eq_texts(output, expected4, sizeof(expected4), "GHASH test 4");

    const uint8_t H5[] = {
        0x4f, 0x62, 0x75, 0xfb, 0x73, 0xb9, 0x8e, 0x5d,
        0x0a, 0x5f, 0x2a, 0xff, 0xcf, 0xd1, 0x9f, 0x4f
    };
    const uint8_t C5[] = {
        0xfe, 0xed, 0xfa, 0xce, 0xde, 0xad, 0xbe, 0xef,
        0xfe, 0xed, 0xfa, 0xce, 0xde, 0xad, 0xbe, 0xef,
        0xab, 0xcd, 0x00, 0x01, 0x11, 0x22, 0x33, 0x44,
        0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc
    };
    const uint8_t expected5[] = {
        0xcd, 0x19, 0x12, 0xec, 0x3c, 0x41, 0x08, 0xf4,
        0x36, 0x56, 0x5d, 0xe5, 0x59, 0xba, 0xc4, 0xd6
    };
    GHASH(H5, C5, sizeof(C5), output, sizeof(output));
    failed |= assert_eq_texts(output, expected5, sizeof(expected5), "GHASH test 5");

    const uint8_t H6[] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02
    };
    const uint8_t expected6[] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
    };
    GHASH(H6, NULL, 0, output, sizeof(output));
    failed |= assert_eq_texts(output, expected6, sizeof(expected6), "GHASH test 6");
